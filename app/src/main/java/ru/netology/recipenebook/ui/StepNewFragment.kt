...
import ru.netology.recipenebook.databinding.StepDetailsEditBinding
            ...
            R.id.recipe_new_options_save -> {
                with(binding) {
                    if (stepContent.text.toString().isBlank()) {
                        Toast.makeText(
                            context,
                            getString(R.string.step_new_string1),
                            Toast.LENGTH_SHORT
                        )
                            .also {
                                it.setGravity(
    ...
    private fun goBackWithDialog() {
        ...
        if (!textSame || pictureAdded) {
            MaterialAlertDialogBuilder(requireContext())
                .setMessage(getString(R.string.step_new_string2))
                .setNegativeButton(getString(R.string.step_new_string3)) { dialog, which ->

                }.setPositiveButton(getString(R.string.step_new_string4)) { dialog, which ->
