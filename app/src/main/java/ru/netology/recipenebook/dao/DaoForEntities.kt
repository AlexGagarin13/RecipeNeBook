...
interface RecipesDao {
    ...
    @Query(
        "SELECT * FROM recipes " +
                "INNER JOIN types ON types.id_cat = recipes.type " +
                "WHERE types.show_or_not = 1"
    )
    ...
    @Query(
        "UPDATE recipes SET " +
                "is_favorite_rec = CASE WHEN :favourite THEN 1 ELSE 0 END " +
                "WHERE id_rec = :id"
    )
...
interface RecStepsDao {
    ...
    @Query(
        "SELECT * FROM recipe_steps " +
                ...
                "INNER JOIN types ON types.id_cat = recipes.type WHERE types.show_or_not = 1 " +
...
interface CategoryDao {
    ...
    @Query("SELECT * FROM types ORDER BY name_cat ASC")
    fun getAllSortedAsc(): Flow<List<RecCategoryEntity>>

    @Query("SELECT * FROM types ORDER BY name_cat DESC")
    fun getAllSortedDesc(): Flow<List<RecCategoryEntity>>

    @Query("SELECT name_cat FROM types WHERE id_cat = :id")
    fun getNameById(id: Long): String?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insert(inCategory: RecCategoryEntity): Long

    @Query("DELETE FROM types WHERE id_cat = :remId")
    fun removeById(remId: Long)

    @Query("DELETE FROM types")
    fun deleteAllTypes()

    @Query("UPDATE types SET show_or_not = 1 WHERE id_cat = :id")
    fun setVisible(id: Long)

    @Query("UPDATE types SET show_or_not = 0 WHERE id_cat = :id")
    fun setNotVisible(id: Long)

    @Query("SELECT id_cat FROM types WHERE name_cat = :category")
    fun getIdByName(category: String?): Long?

    @Query("SELECT COUNT(*) FROM types WHERE show_or_not = 1")
    fun getNumberOfSelectedTypes(): Int
}
